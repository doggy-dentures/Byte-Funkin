...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var funBG = new FlxSprite(0, 50).loadGraphic(Paths.image('thefunny'));
		var funBGagain = new FlxSprite().loadGraphic(Paths.image('thefunnyagain'));
		add(funBGagain);
		...
		grpWeekCharacters.add(new MenuCharacter(430, 25, 0.9, true));
		...
		txtTracklist = new FlxText(10, yellowBG.x + yellowBG.height + 92, 0, "Tracks", 20);
	...
	function selectWeek()
	{
		if (weekUnlocked[curWeek])
		{
			if (stopspamming == false)
			{
				...
				grpWeekCharacters.members[1].animation.play('bytefriend');
				stopspamming = true;
			}
			...
			new FlxTimer().start(1, function(tmr:FlxTimer)
			{
				switch(curWeek)
				{
					...
					case 1:
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/rallycut/rallycut.webm', new PlayState()));
					default: 
